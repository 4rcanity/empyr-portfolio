...
/** Thin auto-reconnecting WebSocket wrapper around the Liar's Bar worker. */
...
const KEY_STORE = 'liars.player.key';
...
export function recallName(): string {
  try {
    return localStorage.getItem('liars.name') ?? '';
  } catch {
    return '';
  }
}
...
export function rememberName(name: string) {
  try {
    localStorage.setItem('liars.name', name);
  } catch {
    /* private mode */
  }
}
